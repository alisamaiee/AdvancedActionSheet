...
internal struct Assets {
    ...
    public static func fetch(fetchLimit: Int? = 0, _ completion: @escaping (FetchResults) -> Void) {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            fetchOptions.fetchLimit = fetchLimit ?? 0
            fetchOptions.sortDescriptors = [NSSortDescriptor(key: "creationDate", ascending: false)]
            fetchOptions.predicate = NSPredicate(format: "mediaType = %d || mediaType = %d", PHAssetMediaType.image.rawValue, PHAssetMediaType.video.rawValue)
            let fetchResult = PHAsset.fetchAssets(with: fetchOptions)//PHAsset.fetchAssets(with: .image, options: PHFetchOptions())
            if fetchResult.count > 0 {
                var assets = [PHAsset]()
                fetchResult.enumerateObjects { object, _, _ in
                    assets.append(object)//.insert(object, at: 0)
                }
